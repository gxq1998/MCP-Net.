...
class NCC_whole:
    ...
    def ncc(self, I, J):
        # get dimension of volume
        # assumes I, J are sized [batch_size, *vol_shape, nb_feats]
        #print('NCC input shape',I.get_shape())
        ndims = len(I.get_shape().as_list()) - 2
        assert ndims in [1, 2, 3], "volumes should be 1 to 3 dimensions. found: %d" % ndims
    ...
    def loss(self, y_true, y_pred):
        #original_shape = y_true.get_shape().as_list()
        #y_true = tf.reshape(y_true,[original_shape[2],original_shape[3],original_shape[4],original_shape[0]*original_shape[5]])
        #y_pred = tf.reshape(y_pred,[original_shape[2],original_shape[3],original_shape[4],original_shape[0]*original_shape[5]])
        #print('sliced y_true shape',y_true[0,0,0,0,:,:].get_shape())
        return - self.ncc(y_true[:,0,:,:,:,:], y_pred[:,0,:,:,:,:])- self.ncc(y_true[:,1,:,:,:,:], y_pred[:,1,:,:,:,:])- self.ncc(y_true[:,2,:,:,:,:], y_pred[:,2,:,:,:,:])- self.ncc(y_true[:,3,:,:,:,:], y_pred[:,3,:,:,:,:])- self.ncc(y_true[:,4,:,:,:,:], y_pred[:,4,:,:,:,:])
        #return - self.ncc(y_true[0,:,:,:,:,:], y_pred[0,:,:,:,:,:])
...
class MSE:
    ...
    def __init__(self, image_sigma=1.0, crop_background = False, clip = False):
    ...
    def loss(self, y_true, y_pred):
        ...
        if self.clip:
            y_pred = K.clip(y_pred, 0, 0.99)
            y_true = K.clip(y_true, 0, 0.99)
    ...
    def loss(self, y_true_, y_pred_):
        ...
        for i in range(5):
            ...
            if self.clip:
                y_pred = K.clip(y_pred, 0, 0.99)
                y_true = K.clip(y_true, 0, 0.99)
            tot_loss += 1.0 / (self.image_sigma**2) * K.mean(K.square(y_true - y_pred))
...
class NMI:
    ...
    def __init__(self, bin_centers, vol_size, sigma_ratio=0.5, max_clip=25, local=False, crop_background=False, patch_size=1):
        """
