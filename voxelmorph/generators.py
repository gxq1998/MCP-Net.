...
def seriesgen(
    ...
    patient_names = list(set([x.split('/')[-1].split('_')[0] for x in vol_names]))
    
    while True:
        # generate [batchsize] random image indices
        patient_indice = np.random.randint(len(patient_names), size=1)[0]
        patient_vol_names = [x for x in vol_names if x.split('/')[-1].split('_')[0]==patient_names[patient_indice]]
        ref_name = [x for x in patient_vol_names if x.split('/')[-1].split('_')[-1].split('.')[0]==str(ref_id)][0]
        #patient_vol_names.remove(ref_name)
        patient_vol_names = patient_vol_names[4:]  # 5-min only
        
        #indices = np.random.randint(len(patient_vol_names), size=batch_size)
        #indices = np.random.permutation(len(patient_vol_names))[:batch_size]
        #np.sort(indices)
        indice = np.random.randint(len(patient_vol_names)-batch_size+1, size=1)[0]
        indices = np.arange(indice,indice+batch_size)

        # load volumes and concatenate
        load_params = dict(np_var=np_var, add_batch_axis=True, add_feat_axis=add_feat_axis, pad_shape=pad_shape, resize_factor=resize_factor)
        imgs = [py.utils.load_volfile(patient_vol_names[i], **load_params) for i in indices]
        refs = [py.utils.load_volfile(ref_name, **load_params) for i in indices]
        vols = [np.concatenate(imgs, axis=0)]
...
def seriesgenwithinfun(
        vol_names,
        ref_id = 11,
        batch_size=1, 
        np_var='vol',
        infun_var='sum_infun',
        this_infun_var='this_infun',
        pad_shape=None,
        resize_factor=1,
        add_feat_axis=True
    ):
    ...
    while True:
        ...
        load_params = dict(np_var=infun_var, add_batch_axis=True, add_feat_axis=add_feat_axis, pad_shape=pad_shape, resize_factor=resize_factor)
        infuns = [py.utils.load_volfile(patient_vol_names[i], **load_params) for i in indices]
        
        load_params = dict(np_var=this_infun_var, add_batch_axis=True, add_feat_axis=add_feat_axis, pad_shape=pad_shape, resize_factor=resize_factor)
        this_infuns = [py.utils.load_volfile(patient_vol_names[i], **load_params) for i in indices]
        
        vols = [np.concatenate(imgs, axis=0)]
        refs = [np.concatenate(refs, axis=0)]
        infuns = [np.concatenate(infuns, axis=0)]
        this_infuns = [np.concatenate(this_infuns, axis=0)]

        yield (vols,refs,infuns,this_infuns)
        
def wholeseriesgenwithinfun(
        vol_names,
        ref_id = 11,
        batch_size = 5,
        np_var='vol',
    ...
    while True:
        ...
        whole_indices = np.arange(len(patient_vol_names))

        indice = np.random.randint(len(patient_vol_names)-batch_size+1, size=1)[0]
        indices = np.arange(indice,indice+batch_size)
        rest_indices = [x for x in whole_indices if x not in indices]
        # load volumes and concatenate
        load_params = dict(np_var=np_var, add_batch_axis=True, add_feat_axis=add_feat_axis, pad_shape=pad_shape, resize_factor=resize_factor)
        imgs = [py.utils.load_volfile(patient_vol_names[i], **load_params) for i in indices]
        refs = [py.utils.load_volfile(ref_name, **load_params) for i in indices]
        
        load_params = dict(np_var=rest_var, add_batch_axis=True, add_feat_axis=add_feat_axis, pad_shape=pad_shape, resize_factor=resize_factor)
        #load_params = dict(np_var=np_var, add_batch_axis=True, add_feat_axis=add_feat_axis, pad_shape=pad_shape, resize_factor=resize_factor)
        rest_imgs = [py.utils.load_volfile(patient_vol_names[i], **load_params) for i in rest_indices]
        load_params = dict(np_var=np_var, add_batch_axis=True, add_feat_axis=add_feat_axis, pad_shape=pad_shape, resize_factor=resize_factor)
        rest_imgs.append(py.utils.load_volfile(ref_name, **load_params))
        
        load_params = dict(np_var=infun_var, add_batch_axis=True, add_feat_axis=add_feat_axis, pad_shape=pad_shape, resize_factor=resize_factor)
        infuns = [py.utils.load_volfile(patient_vol_names[i], **load_params) for i in indices]
        rest_infuns = [py.utils.load_volfile(patient_vol_names[i], **load_params) for i in rest_indices]
        infuns.extend(rest_infuns)
        infuns.append(py.utils.load_volfile(ref_name, **load_params))
        
        load_params = dict(np_var=this_infun_var, add_batch_axis=True, add_feat_axis=add_feat_axis, pad_shape=pad_shape, resize_factor=resize_factor)
        this_infuns = [py.utils.load_volfile(patient_vol_names[i], **load_params) for i in indices]
        rest_this_infuns = [py.utils.load_volfile(patient_vol_names[i], **load_params) for i in rest_indices]
        this_infuns.extend(rest_this_infuns)
        this_infuns.append(py.utils.load_volfile(ref_name, **load_params))
        
        vols = [np.concatenate(imgs, axis=0)]
        refs = [np.concatenate(refs, axis=0)]
        rest_imgs = [np.concatenate(rest_imgs, axis=0)]
        infuns = [np.concatenate(infuns, axis=0)]
        this_infuns = [np.concatenate(this_infuns, axis=0)]

        yield (vols,refs,rest_imgs,infuns,this_infuns)

        
def scan_series(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    ...
    """
    zeros = None
    gen = seriesgen(vol_names, batch_size=batch_size, **kwargs)
    while True:
...
def scan_series_with_infun(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    Generator for scan-to-scan registration.
    ...
    """
    zeros = None
    gen = seriesgenwithinfun(vol_names, batch_size=batch_size, **kwargs)
    while True:
        scan1, scan2, infun, this_infun = next(gen)
        scan1 = scan1[0]
        scan2 = scan2[0]
        infun = infun[0]
        this_infun = this_infun[0]
        ...
        invols  = [scan1, scan2, infun, this_infun]
        outvols = [scan2, scan1] if bidir else [scan2]
        #thresh = 0.001
        #mask = scan2 > thresh
        #zeros_for_patlak = mask * thresh
        outvols.append(zeros)
        if not no_warp:
            outvols.append(zeros)
...
def scan_whole_series_with_infun(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    Generator for scan-to-scan registration.
    ...
    """
    zeros = None
    gen = wholeseriesgenwithinfun(vol_names, batch_size=batch_size, **kwargs)
    while True:
        scan1, scan2, scan3, infun, this_infun = next(gen)
        scan1 = scan1[0][np.newaxis,:,:,:,:,:]
        scan2 = scan2[0][np.newaxis,:,:,:,:,:]
        scan3 = scan3[0][np.newaxis,:,:,:,:,:]
        infun = infun[0][np.newaxis,:,:,:,:,:]
        this_infun = this_infun[0][np.newaxis,:,:,:,:,:]

        # cache zeros
        if not no_warp and zeros is None:
            shape = scan1.shape[1:-1]
            shape_flow = [int(x/2) for x in shape]
            shape_flow[0] = shape[0]
            zeros = np.zeros((1,*shape_flow, len(shape_flow)-1))
            zeros_estimation = np.zeros((1,*shape, 1))

        invols  = [scan1, scan2, scan3, infun, this_infun]
        outvols = [scan2, scan1] if bidir else [scan2]

        outvols.append(zeros_estimation)
        if not no_warp:
            outvols.append(zeros)
