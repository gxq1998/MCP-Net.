...
#valid_vol_names = glob.glob(os.path.join(args.validation, '*.npz'))
...
if args.atlas:
    ...
    #v_generator = vxm.generators.scan_to_scan(valid_vol_names, batch_size=args.batch_size, bidir=args.bidir, add_feat_axis=add_feat_axis)
    generator = vxm.generators.scan_series(train_vol_names, batch_size=args.batch_size, bidir=args.bidir, add_feat_axis=add_feat_axis)
    v_generator = vxm.generators.scan_series(valid_vol_names, batch_size=args.batch_size, bidir=args.bidir, add_feat_axis=add_feat_axis)
...
# unet architecture
enc_nf = args.enc if args.enc else [16, 32, 32, 32] #whole-body [16, 32, 32]; vxm default [16, 32, 32, 32]
dec_nf = args.dec if args.dec else [32, 32, 32, 32, 32, 16, 16] #whole-body [32, 32, 32, 16, 16]; vxm default [32, 32, 32, 32, 32, 16, 16]
...
#    print('tf.device(device)',tf.device(device))
    ...
    # build the model
    model = vxm.networks.VxmDenseTemporal(
        inshape=inshape,
    ...
    if args.image_loss == 'ncc':
        ...
        image_loss_func = vxm.losses.MSE(args.image_sigma, crop_background=True).loss
    elif args.image_loss == 'nmi':
        bin_centers, vol_size = np.linspace(0.001, 10, num=64, retstep=True)
        image_loss_func = vxm.losses.NMI(bin_centers, vol_size, crop_background=True).loss
    else:
        raise ValueError('Image loss should be "mse", "ncc" or "nmi", but found "%s"' % args.image_loss)
    ...
    model.fit_generator(generator,
        ...
        epochs=args.epochs,
        steps_per_epoch=int(len(train_vol_names)/args.batch_size), #args.steps_per_epoch,
    ...
    #model.evaluate_generator(v_generator)
